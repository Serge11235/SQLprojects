...
GO
	...
	declare @CurrRow int = 11;
	declare @CurrPlace int = 14;
...
--СУММА С ПРОДАЖ ПО ПРЕДСТАВЛЕНИЯМ (в выходной нет скидок кроме коэфф. самого выходного дня)USE [CircusAuditDB]
...
GO
--СУММА С ПРОДАЖ ПО ПРЕДСТАВЛЕНИЯМ (с учётом приоритетов)
...
--СУММА С ПРОДАЖ ПО ПРЕДСТАВЛЕНИЯМ (всё коэфф. включены в любой день)
/*USE [CircusAuditDB]
GO
SELECT DISTINCT Id_Ticket, (SELECT Name FROM Posters WHERE Id_poster=Id_event) as EventName, Event_date, 
			((SELECT Prise FROM Posters Where Id_poster=Id_event)*
...
--ОТЧЁТ ПО ДНЯМ (выручка с детей)
/*
GO
SELECT Id_TicketFK FROM Modifier WHERE Id_Type=5;

SELECT Id_Ticket, (SELECT Id_Meaning FROM Modifier_Set WHERE Id_type =Id_MS) as status, Event_date, 
			((SELECT Prise FROM Posters Where Id_poster=Id_event)*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=Id_Ticket AND PriorityType =1))*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=Id_Ticket AND PriorityType =2))*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=Id_Ticket AND PriorityType =3))*
	...
	FROM Tickets, Modifier, Modifier_Set
	WHERE (Modifier_Set.Id_MS in (Modifier.Id_Type)) AND
			(Id_Ticket in (Modifier.Id_TicketFK)) AND Id_Type = 5
*/

--Получить цену билета в таблицу
/*
...
GO
	INSERT INTO PriceTab VALUES (2, (SELECT Prise FROM Posters Where Id_poster=1)*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=2 AND PriorityType =1))*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=2 AND PriorityType =2))*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=2 AND PriorityType =3))*
			(SELECT PercentValue FROM Modifier_Set Where Id_MS=(select Id_type from Modifier where Id_TicketFK=2 AND PriorityType =4))
			);
*/
